...
window.SharakahAIGetAppCheckToken=async({forceRefresh=false}={})=>{
  const result=await getToken(appCheck,forceRefresh);
  if(!result?.token)throw new Error('sharakah-app-check-token-missing');
  return result.token;
};
window.SHARAKAH_AI_TOKEN_ENDPOINT='https://mesraah-live-token.naif123456.workers.dev/token';
